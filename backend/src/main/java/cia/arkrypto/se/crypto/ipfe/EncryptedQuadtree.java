...
public class EncryptedQuadtree {
    ...
    public QuadtreeNode creatNode(String m, Point p){
        ...
        return IPFESystem.keygen(Zr, x, p, m);
    }
...
//            System.out.println(n.getM());
    ...
    public Ciphertext getCiphertext(Point p){
        ...
        return IPFESystem.encrypt(HashUtil.hashStr2ZrArr(Zr, m, l), r);
    }
    ...
    public Element innerProduct(Element[] x, Element[] y){
        return IPFESystem.innerProduct(Zr, x, y);
    }
    ...
    public QuadtreeNode search(QuadtreeNode node, Ciphertext ciphertext, Point p){
        if(node == null || ciphertext == null || p == null){
            return null;
        }
    ...
    public void test(){
        ...
        QuadtreeNode node = IPFESystem.keygen(Zr, x, null, null);
        Element r = Zr.newRandomElement().getImmutable();
        Ciphertext ciphertext = IPFESystem.encrypt(y, r);

        // 再用 C, D, Ei 算 Ex
        Element Ex = IPFESystem.decrypt(G1, node, ciphertext);
